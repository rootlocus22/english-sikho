...
export default function SocialProofNotification() {
    ...
    return (
        <Card className="fixed bottom-2 right-2 sm:bottom-4 sm:right-4 z-50 w-[calc(100vw-1rem)] sm:w-80 md:w-96 max-w-[calc(100vw-1rem)] bg-white border-2 border-green-200 shadow-2xl animate-in slide-in-from-bottom-5">
            <div className="p-3 sm:p-4">
                <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-green-600" />
                        <span className="text-[10px] sm:text-xs font-semibold text-green-600 uppercase">Live Activity</span>
                    </div>
                    <button
                ...
                <p className="text-[10px] sm:text-xs text-slate-500 mt-1">{notification.time}</p>
            </div>
        </Card>
    );
}
